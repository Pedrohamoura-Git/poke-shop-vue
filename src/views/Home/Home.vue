...
<template>
 <main class="home" id="scroll_to_me">
  <section class="home__content">
  <template v-for="(poke, index) in paginationData" :key="index">
    <PokeballCard 
      :type="poke.type.pt_br" 
      :imageUrl="poke.image.url" 
      @click="handleSelectedType(poke.type.en_us)"
    />
  </template>
  </section>
  <section class="pagination_container" v-if="pokemon_types_list.length !== 0">
   <Pagination
...
export default {
 ...
 methods: {
  handleSelectedType(type) {
   ...
   this.$router.push({name: 'TypeList', params: {selectedType: type}})
  },
